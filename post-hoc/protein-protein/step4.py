...
def co_occur_pair(file_name='binom_cooccur.txt'):
    ...
    for i in range(len(sig_pvals)): # We are only looking at top 5360 significant pairs
        ...
        tomtom_a_tsv = pd.read_csv(f'../../saved_model/MuSeAM_regression/motif_files/motif_files/tomtom_outputs_evalue_5_v2/result_{"{0:0=3d}".format(pair_a_num)}/tomtom.tsv', sep='\t')
        ...
        tomtom_b_tsv = pd.read_csv(f'../../saved_model/MuSeAM_regression/motif_files/motif_files/tomtom_outputs_evalue_5_v2/result_{"{0:0=3d}".format(pair_b_num)}/tomtom.tsv', sep='\t')
