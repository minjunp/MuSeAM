...
def co_occur_pair(file_name='binom_cooccur.txt'):
    ...
    sig_pvals = [i for i in binom_pvals if i < 0.05] # 2272 significant pairs
    ...
    for i in range(len(sig_pvals)): # We are only looking at top 5360 significant pairs
        ...
        #tomtom_a_tsv = pd.read_csv(f'/Users/minjunpark/Documents/MuSeAM/post-hoc/regulators/tomtom/tomtom_outputs_evalue_10/result_{"{0:0=3d}".format(pair_a_num)}/tomtom.tsv', sep='\t')
        ...
        #tomtom_b_tsv = pd.read_csv(f'/Users/minjunpark/Documents/MuSeAM/post-hoc/regulators/tomtom/tomtom_outputs_evalue_10/result_{"{0:0=3d}".format(pair_b_num)}/tomtom.tsv', sep='\t')
