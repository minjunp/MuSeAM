...
class Museam:
    ...
    def create_model(self):
        ...
        # Multinomial Layer
        first_layer = ConvolutionLayer(filters=300, 
                                       kernel_size=19, 
                                       strides=1, 
        ...
        if self.pool_type == 'Max':
            pool_layer_1 = MaxPooling1D(pool_size=3)(concat_relu)
        ...
        # Normal Conv1D
        Normal_Conv1D_1 = Conv1D(filters=200,kernel_size=11)(pool_layer_1)
        Relu_layer_2 = ReLU()(Normal_Conv1D_1)
        pool_layer_2 = MaxPooling1D(pool_size=4)(Relu_layer_2)
        Normal_Conv1D_2 = Conv1D(filters=200,kernel_size=7)(pool_layer_2)
        Relu_layer_3 = ReLU()(Normal_Conv1D_2)



        # Flatten Layer (None, 512)
        flat = Flatten()(Relu_layer_3)

        
        # Dense Layers
        Dense_Layer1 = Dense(1000,activation= 'relu')(flat)
        Dense_Layer2 = Dense(200,activation= 'relu')(Dense_Layer1)
        

        # Output Layers
        if self.activation_type == 'linear':
            if self.regularizer == 'L_1':
                outputs = Dense(1, kernel_initializer='normal', kernel_regularizer=regularizers.l1(0.001), activation= self.activation_type)(Dense_Layer2)
            elif self.regularizer == 'L_2':
                outputs = Dense(1, kernel_initializer='normal', kernel_regularizer=regularizers.l2(0.001), activation= self.activation_type)(Dense_Layer2)
            else:
                raise NameError('Set the regularizer name correctly')
        elif self.activation_type =='sigmoid':
            outputs = Dense(1, activation= self.activation_type)(Dense_Layer2)
