...
def run_model(argv = None):
    ...
    eval(parameters,fasta_file,readout_file)
...
def create_model(dim_num):
    ...
    #First Conv1D
    model.add(tf.keras.layers.Conv1D(filters=120, 
...
def eval(parameters,fasta_file,readout_file):
    # Preprocess the data
    ...
    history = model.fit(forward_shuffle, readout_shuffle, epochs=parameters['epochs'], batch_size=parameters['batch_size'], validation_split=parameters['validation_split'])
